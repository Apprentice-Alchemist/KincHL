...
MAKE_GET_SET_EX(g5_render_target, contextId, int, _I32)
MAKE_GET_SET_EX(g5_render_target, isCubeMap, bool, _BOOL)
MAKE_GET_SET_EX(g5_render_target, isDepthAttachment, bool, _BOOL)

HL_PRIM void HL_NAME(hl_g5_render_target_init)(hl_g5_render_target *renderTarget, int width, int height, int depthBufferBits, bool antialiasing,
                                               kinc_g5_render_target_format_t format, int stencilBufferBits, int contextId) {
    kinc_g5_render_target_init(&renderTarget->t, width, height, depthBufferBits, antialiasing, format, stencilBufferBits, contextId);
}
HL_PRIM void HL_NAME(hl_g5_render_target_init_cube)(hl_g5_render_target *renderTarget, int cubeMapSize, int depthBufferBits, bool antialiasing,
                                                    kinc_g5_render_target_format_t format, int stencilBufferBits, int contextId) {
    kinc_g5_render_target_init_cube(&renderTarget->t, cubeMapSize, depthBufferBits, antialiasing, format, stencilBufferBits, contextId);
}
...
DEFINE_PRIM(_VOID, hl_g5_render_target_init, _RENDER_TARGET _I32 _I32 _I32 _BOOL _I32 _I32 _I32)
DEFINE_PRIM(_VOID, hl_g5_render_target_init_cube, _RENDER_TARGET _I32 _I32 _BOOL _I32 _I32 _I32)
